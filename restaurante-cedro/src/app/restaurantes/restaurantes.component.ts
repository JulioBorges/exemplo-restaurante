...
export class RestaurantesComponent implements OnInit {
  ...
  constructor(private restaurantesService: RestaurantesService, private alertDialog: AlertDialogService) { }
  ...
  chamarExclusao(restaurante: Restaurante): void {  
    ...
    `Confirma a exclusão do restaurante ${restaurante.nome}?`).subscribe(resultado => {
  ...
  pesquisar() {
    if (this.nomePesquisado) {
      ...
      this.restaurantesService.searchRestaurantes(this.nomePesquisado)
        .subscribe(data => {
          this.dataSource.data = data;
          this.processando = false;
        });
